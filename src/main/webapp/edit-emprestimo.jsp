<%@page import="br.com.infnet.blocojava.appfinanceiro.services.ContaService"%>
<%@page import="br.com.infnet.blocojava.appfinanceiro.business.Conta"%>
...
<body>
	...
	<%
		...
		List<Conta> contas = (List<Conta>)request.getAttribute("contaList");
	%>
	...
	<div class="container">
		...
		<form action="<%=destiny%>" method="POST">
			<% for(String fieldName: fields.keySet()) { %>
				...
				<%
	    		if(!fieldData.getTitle().equalsIgnoreCase("conta"))
	    		{
	    		%>
			      <label for="<%=fieldName %>"><%=fieldData.getTitle()%>:</label>
			    	...
			    	<%
			    		...
			    		else if(fieldData.getType().isPrimitive() || fieldData.getType().equals(Integer.class))
			    		{
			...
			<div class="form-group">
			    <label for="listaFavorecidosSelect">Conta</label>
			    <select class="form-control" id="contaId" name="contaId">
			      <% for(Conta conta: contas) { %>
						<option value="<%=conta.getId()%>" id="contaId" name="contaId"><%=conta.valorDescricao()%></option>  
					<% } %>
				</select>
			</div>
